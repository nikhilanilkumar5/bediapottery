...
export default async function SuccessPage({ searchParams }: SuccessPageProps) {
  ...
  const customerName =
    firstBooking?.customer.firstName + " " + firstBooking?.customer.lastName ||
    "N/A";
      ...
      <div className="w-full bg-secondary-dark py-8 mb-12 border-t border-b border-[#e5e1d8]">
        <div className="max-w-7xl mx-auto flex items-center justify-center px-4">
          <button className="flex items-center gap-3">
            ...
            <span className="font-medium hidden sm:block text-black">
              Shopping Cart
            </span>
          </button>
          ...
          <>
            ...
            <div className="flex justify-between items-end mb-8 border-b border-gray-200 pb-7">
              ...
              <span className="inline-flex items-center gap-2 text-[#0D463D] font-medium">
                ...
                >
                  ...
                  <path
                    d="M11.8145 16.5009L14.9345 19.6209L21.1855 13.3809"
                    stroke="white"
                    strokeWidth="1.5"
                    strokeLinecap="round"
            ...
            <div className="space-y-8  lg:block hidden">
              {orderdata.items.map((item, index) => (
                <div
                  key={index}
                  className="flex  flex-col  sm:flex-row items-center gap-6 border-b border-gray-200 pb-8 justify-start"
                >
                  <div className="w-24 h-24 bg-gray-200 rounded-sm shrink-0 overflow-hidden relative">
                    <Image
                      src={
                        item.bookingId.workshopId.images?.[0]?.image ||
                    ...
                    <div className="flex  items-start mb-2">
                      <div className="w-5/12">
                        <h3 className="font-normal  text-black text-[15px]">
                          {item.bookingId.workshopId.title}-
                          {item.bookingId.items[0]?.optionTitle}
                        </h3>
                      </div>
                      <div className="w-7/12 flex justify-between items-start">
                        <div className="text-sm text-gray-500 space-y-1">
                          {(item.bookingId.items[0]?.wheelPottery ?? 0) > 0 && (
                            <p>
                              x {item.bookingId.items[0]?.wheelPottery} Wheel
                              Pottery
                              <span className="ml-1 text-black">
                                {item.bookingId.currency}{" "}
                                {Number(
                                  item.bookingId.items[0]?.price ?? 0,
                                ).toFixed(2)}
                              </span>
                            </p>
                          )}
                          {(item.bookingId.items[0]?.handBuild ?? 0) > 0 && (
                            <p>
                              x {item.bookingId.items[0]?.handBuild}{" "}
                              HandBuilding
                              <span className="ml-1 text-black">
                                {item.bookingId.currency}{" "}
                                {item.bookingId.items[0]?.price.toFixed(2)}
                              </span>
                            </p>
                          )}
                          <p>
                        ...
                        <p>
                          <span className="ml-1 text-gray-500">
                            {item.bookingId.currency}{" "}
                            {item.bookingId.items[0]?.subtotal.toFixed(2)}
                          </span>
                        </p>
                      </div>
                    </div>
            ...
            <div className="flex justify-between items-end py-2 pt-7">
              ...
              <span className=" text-black font-medium">
                {orderdata.currency} {Number(orderdata.subtotal).toFixed(2)}
              </span>
            </div>
            ...
            <div className="flex justify-between items-end  border-b border-gray-200 py-2 pb-7">
              ...
              <span className=" text-black font-medium">
                {orderdata.currency} {Number(orderdata.grandTotal).toFixed(2)}
              </span>
            </div>
            ...
            <div className=" border-b border-gray-200 py-6 mb-10">
              <div className="grid grid-cols-1 gap-8 ">
                ...
                <div className="space-y-4">
                  <div className="flex justify-between items-start md:gap-16">
                  ...
                  <div className="flex justify-between items-start gap-4">
                    ...
                    <span className="text-black font-medium text-right max-w-60 ml-auto break-words">
                 {customerAddress}
                    </span>
                  </div>
                </div>
